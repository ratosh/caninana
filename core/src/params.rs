pub const MAX_WORKERS: usize = 60;
pub const DOUBLE_EVOLUTION_WORKERS: usize = 54;
pub const DOUBLE_GAS_PER_BASE_WORKERS: usize = 38;
pub const UNLOCK_ROACH_WORKERS: usize = 26;
pub const UNLOCK_HYDRA_WORKERS: usize = 45;
pub const UNLOCK_LATE_TECH_WORKERS: usize = 54;
pub const UNLOCK_OVERSEER_WORKERS: usize = 29;
pub const UNLOCK_BURROW_WORKERS: usize = 28;
pub const OVERLORD_SPEED_WORKERS: usize = 30;

pub const CREEP_DISTANCE_TO_HALL: f32 = 5f32;
pub const CREEP_SPREAD_MAP_DISTANCE: usize = 4;
pub const CREEP_SPREAD_DISTANCE: usize = 5;
pub const CREEP_SPREAD_ENERGY: u32 = 125;

pub const TRANSFUSION_MIN_ENERGY: u32 = 50;
pub const TRANSFUSION_MAX_RANGE: f32 = 15f32;
pub const TRANSFUSION_MISSING_HEALTH: u32 = 60;

pub const BURROW_HEALTH_PERCENTAGE: f32 = 0.5f32;
pub const BURROW_DETECTION_DISTANCE: f32 = 11f32;
pub const UNBURROW_HEALTH_PERCENTAGE: f32 = 0.9f32;
